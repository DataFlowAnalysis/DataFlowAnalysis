...
public class PCMDataCharacteristicsCalculator implements DataCharacteristicsCalculator {
   ...
   */
  public PCMDataCharacteristicsCalculator(
      List<DataFlowVariable> initialVariables,
  ...
  public void evaluate(ConfidentialityVariableCharacterisation variableCharacterisation) {
    ...
    EnumCharacteristicType characteristicType =
        (EnumCharacteristicType) leftHandSide.getCharacteristicType();
    ...
    DataFlowVariable existingVariable =
        this.getDataFlowVariableByReference(reference)
            .orElse(new DataFlowVariable(reference.getReferenceName()));

    List<CharacteristicValue> modifiedCharacteristics =
        calculateModifiedCharacteristics(existingVariable, characteristicType, characteristicValue);

    DataFlowVariable modifiedVariable =
        createModifiedDataFlowVariable(existingVariable, modifiedCharacteristics, rightHandSide);
   ...
   */
  private Optional<DataFlowVariable> getDataFlowVariableByReference(
      AbstractNamedReference reference) {
   ...
   */
  private DataFlowVariable createModifiedDataFlowVariable(
      DataFlowVariable existingVariable,
      List<CharacteristicValue> modifiedCharacteristics,
      Term rightHandSide) {
    DataFlowVariable computedVariable = new DataFlowVariable(existingVariable.variableName());
    var unmodifiedCharacteristics =
        existingVariable.getAllCharacteristics().stream()
    ...
    for (CharacteristicValue modifiedCharacteristic : modifiedCharacteristics) {
      if (evaluateTerm(rightHandSide, modifiedCharacteristic)) {
        List<CharacteristicValue> modifiedCharacteristicValues =
            computedVariable.getAllCharacteristics().stream()
   ...
   */
  private List<CharacteristicValue> calculateModifiedCharacteristics(
      DataFlowVariable existingVariable,
      EnumCharacteristicType characteristicType,
    ...
    if (characteristicValue == null && characteristicType != null) {
      ...
      return List.of(
          existingVariable.getAllCharacteristics().stream()
              .filter(it -> it.getValueName().equals(characteristicValue.getName()))
              .filter(it -> it.getTypeName().equals(characteristicType.getName()))
              .findAny()
   ...
   *         undefined)
   */
  private boolean evaluateNamedReference(
      NamedEnumCharacteristicReference characteristicReference,
      CharacteristicValue characteristicValue) {
    var optionalDataflowVariable =
        getDataFlowVariableByReference(characteristicReference.getNamedReference());
    if (optionalDataflowVariable.isEmpty()) {
      return false;
    }
    var dataflowVariable = optionalDataflowVariable.get();
    var characteristicReferenceTypeName =
    ...
    var characteristicReferenceValueName =
        characteristicReference.getLiteral() != null
            ? characteristicReference.getLiteral().getName()
            : characteristicValue.getValueName();

    var characteristic =
        dataflowVariable.getAllCharacteristics().stream()
            .filter(it -> it.getTypeName().equals(characteristicReferenceTypeName))
            .filter(it -> it.getValueName().equals(characteristicReferenceValueName))
            .findAny();
   ...
   *         bound
   */
  private List<CharacteristicValue> discoverNewVariables(
      DataFlowVariable variable, Optional<EnumCharacteristicType> characteristicType) {
    ...
    List<EnumCharacteristicType> characteristicTypes =
        dataDictionaries.stream()
            .flatMap(it -> it.getCharacteristicTypes().stream())
            .filter(
                it ->
                    characteristicType.isEmpty()
                        || it.getName().equals(characteristicType.get().getName()))
            ...
            .collect(
                Collectors.collectingAndThen(
                    Collectors.toCollection(
                        () ->
                            new TreeSet<EnumCharacteristicType>(
                                Comparator.comparing(EnumCharacteristicType::getName))),
        ...
        .forEach(
            enumCharacteristicType ->
                enumCharacteristicType
                    ...
                    .forEach(
                        characteristicValue ->
                            updatedCharacteristicValues.add(
                                new PCMCharacteristicValue(
                                    enumCharacteristicType, characteristicValue))));
