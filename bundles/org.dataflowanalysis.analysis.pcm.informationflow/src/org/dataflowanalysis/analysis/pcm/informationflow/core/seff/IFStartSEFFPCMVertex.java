...
/**
 * A concrete implementation of {@link AbstractIFSEFFPCMVertex} with the underlying SEFF element type
 * {@link StartAction}. If the {@code StartAction} is directly in the SEFF of a {@link GuardedBranchTransition} this
 * vertex also handles the calling evaluation of the logically previous {@code GuardedBranchTransition}. Note, this is
 * only relevant in case of handling implicit flow.
 */
public class IFStartSEFFPCMVertex extends AbstractIFSEFFPCMVertex<StartAction> {

    /**
     * As for a {@link SEFFPCMVertex} the vertex has an underlying StartAction SEFF element which influences the behavior
     ...
     */
    public IFStartSEFFPCMVertex(StartAction element, List<? extends AbstractPCMVertex<?>> previousElements, Deque<AssemblyContext> context,
        ...
        super(element, previousElements, context, parameter, resourceProvider, considerImplicitFlow, extractionStrategy);
    }

    @Override
    protected AbstractIFSEFFPCMVertex<StartAction> createIFSEFFVertex(StartAction element, List<? extends AbstractPCMVertex<?>> previousElements,
    ...
    protected List<DataFlowVariable> modifyIncomingDataFlowVariables(List<DataFlowVariable> incomingDataFlowVariables) {
        ...
        List<DataFlowVariable> securityContextLayers = IFSecurityContextUtils.getAllSecurityContextLayers(incomingVariables);
    ...
    private List<DataFlowVariable> evaluateOutgoingDataFlowVariablesOfBranchTransition(List<DataFlowVariable> incomingVariables) {
        var supposedGuardedBranchTransition = getReferencedElement().eContainer()
                .eContainer();
        if (supposedGuardedBranchTransition instanceof GuardedBranchTransition branchTransition) {

            List<ConfidentialityVariableCharacterisation> confidentialityCharacterisations = extractConfidentialityCharacterisationsOfGuardedBranchTransition(
                    branchTransition);
            List<DataFlowVariable> outgoingVariables = getDataFlowVariables(getVertexCharacteristics(), confidentialityCharacterisations,
                    incomingVariables);
            return outgoingVariables;
        }
    ...
    private List<ConfidentialityVariableCharacterisation> extractConfidentialityCharacterisationsOfGuardedBranchTransition(
        ...
        Expression condition = branchTransition.getBranchCondition_GuardedBranchTransition()
                .getExpression();
        ...
        return getExtractionStrategy().calculateConfidentialityVariableCharacterisationForExpression(nextSecurityContextName, condition,
                oldSecurityContext);
