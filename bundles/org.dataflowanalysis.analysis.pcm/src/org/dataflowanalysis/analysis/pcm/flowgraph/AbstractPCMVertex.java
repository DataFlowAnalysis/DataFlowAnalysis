...
public abstract class AbstractPCMVertex<T extends Entity> extends AbstractVertex<T> {
   ...
   */
  @Override
  protected void setPropagationResult(
      List<DataFlowVariable> incomingDataFlowVariables,
  ...
  protected List<CharacteristicValue> getVertexCharacteristics() {
    VertexCharacteristicsCalculator vertexCharacteristicsCalculator =
        new PCMNodeCharacteristicsCalculator(this.resourceProvider);
    return vertexCharacteristicsCalculator.getNodeCharacteristics(
        this.referencedElement, this.context);
  }

  /**
   * Calculate the data characteristics for the vertex with the given data characteristics calculator, vertex characteristics, variable characterizations and old data flow variables
   * @param dataCharacteristicsCalculatorFactory Data characteristics factory that is used to calculate the data characteristics at the present vertex
   ...
   */
  protected List<DataFlowVariable> getDataFlowVariables(
      List<CharacteristicValue> vertexCharacteristics,
   ...
   */
  protected void checkCallParameter(
      OperationSignature callSigniture,
      List<ConfidentialityVariableCharacterisation> variableCharacterisations) {
    List<String> parameter =
        callSigniture.getParameters__OperationSignature().stream()
    ...
    List<String> referencedParameter =
        variableCharacterisations.stream()
            .map(
                it ->
                    it.getVariableUsage_VariableCharacterisation()
            ...
            it -> {
              logger.warn(
                  "Unknown reference to variable "
                      + it
                      + " in variable characterisation in vertex "
