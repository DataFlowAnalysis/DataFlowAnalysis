...
public class ConstraintTest extends BaseTest {
  ...
  protected void printViolation(List<? extends AbstractVertex<?>> dataFlowQueryResult) {
    dataFlowQueryResult.forEach(
        it ->
            logger.debug(
                String.format(
                    "Constraint violation found: %s", createPrintableNodeInformation(it))));
  }
  ...
  protected String createPrintableNodeInformation(AbstractVertex<?> node) {
    ...
    String nodeCharacteristics =
        createPrintableCharacteristicsList(node.getAllNodeCharacteristics());
    String dataCharacteristics =
        node.getAllDataFlowVariables().stream()
            .map(
                e ->
                    String.format(
                        "%s [%s]",
