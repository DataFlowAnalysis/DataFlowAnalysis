...
/**
 * This interface represents the functionality of a data flow confidentiality analysis.
 * To use the analysis the {@link DataFlowConfidentialityAnalysis#initializeAnalysis()} method must be called.
 * After that the flow graph of the model can be determined with {@link DataFlowConfidentialityAnalysis#findFlowGraph()}.
 * To determine characteristics at each node the method {@link DataFlowConfidentialityAnalysis#evaluateFlowGraph(FlowGraph)} must be called.
 * Finally, a constraint can be evaluated with {@link DataFlowConfidentialityAnalysis#queryDataFlow(AbstractPartialFlowGraph, Predicate)} 
 * on each partial flow graph contained in the previously returned flow graph.
 * TODO: Naming: Confidentiality required here? Can´t this also analyse something else?
 *
 */
public interface DataFlowConfidentialityAnalysis {
     ...
     * @param condition Condition that describes a violation at one vertex. If the condition returns true, the condition is violated and the vertex is included in the output.
