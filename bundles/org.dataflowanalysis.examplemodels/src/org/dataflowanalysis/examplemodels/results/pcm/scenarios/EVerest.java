...
public class EVerest implements PCMExampleModelResult {
    ...
    /**
     * Builds an AnalysisConstraint item that can check if data with data status labels and without data status labels
     * flow to specific locations
     ...
     */
    public static AnalysisConstraint withStatusWithoutStatusNeverFlowsTo(List<String> withStatus,
            List<String> withoutStatus, List<String> locations) {
    ...
    /**
     * Builds an AnalysisConstraint item that checks if data with status labels and without status labels flows to
     * specific components
     ...
     */
    public static AnalysisConstraint withStatusWithoutStatusNeverFlowsToComponent(List<String> withStatus,
            List<String> withoutStatus, List<String> components) {
        var constraint = new ConstraintDSL().ofData()
                .withLabel("Status", withStatus)
                .withoutLabel("Status", withoutStatus)
                .fromNode()
    ...
    /**
     * Builds an AnalysisConstraint item that ensures data with specific status labels can only flow to specified
     * locations.
    ...
    /**
     * Builds an AnalysisConstraint item that ensures only specified actors have access to data with specific status
     * labels.
     ...
     */
    public static AnalysisConstraint onlyActorWithStatusHasAccessToComponent(List<String> actorStatus,
    ...
    public static AnalysisConstraint onlyActorWithStatusHasAccessToLocation(List<String> actorStatus,
     ...
     * @return the AnalysisConstraint item that can be used for the data flow and access analysis
     */
    public static AnalysisConstraint onlyActorWithStatusHasAccessToDataWithStatusWithoutStatus(List<String> actorStatus,
            List<String> withStatus, List<String> withoutStatus) {
        var constraint = new ConstraintDSL().ofData()
                .withoutLabel("ActorStatus", actorStatus)
                .withLabel("Status", withStatus)
                .withoutLabel("Status", withoutStatus)
    ...
    public List<AnalysisConstraint> getDSLConstraints() {
        ...
        constraints.add(withStatusWithoutStatusNeverFlowsTo(List.of("Token"), List.of("Hashed", "Encrypted"),
                List.of("TPM", "LocalStorage", "External")));
        constraints.add(withStatusWithoutStatusNeverFlowsToComponent(List.of("Token"), List.of("Hashed, Encrypted"),
                List.of("Logs")));

        // Requirement #25:
        // The OCPP modules shall store only the hash of the authorization token of an EV user persistently
        constraints.add(withStatusWithoutStatusNeverFlowsTo(List.of("Token"), List.of("Hashed"),
                List.of("LocalStorage", "TPM")));
        constraints.add(
                withStatusWithoutStatusNeverFlowsToComponent(List.of("Token"), List.of("Hashed"), List.of("Logs")));

        // Requirement #26:
        // The OCPP modules and the authorization related modules (Auth, TokenProvider, TokenValidator) shall ensure
        // that no
        ...
        // Requirement #32:
        // The security certs should be stored securely in a hardware module that doesn't give access to the private
        // keys.
        constraints
                .add(withStatusWithoutStatusNeverFlowsTo(List.of("Certificate"), List.of("Encrypted"), List.of("TPM")));
        ...
        // Requirement #37:
        // Authentication tokens & payment information should be provided to the relevant parties (CSMS, payment
        // provider) and
        ...
        constraints
                .add(onlyFlowsTo(List.of("PaymentInformation"), List.of("CSMS", "PaymentProvider", "ChargingStation")));
        ...
        // Requirement #39:
        // Everest needs to make sure that data that has been transferred to cloud systems like payment providers is not
        // written
        ...
        // Requirement #46:
        // The system (the whole charger) should not hold PII data unencrypted at rest. eg. mac addresses in log files
        // (but a
        // mac address as PII is debatable).
        constraints.add(withStatusWithoutStatusNeverFlowsToComponent(List.of("Sensitive"), List.of("Encrypted"),
                List.of("Logs")));

        // Requirement #60:
        // No identifying information is available to unauthorized persons.
        constraints.add(onlyActorWithStatusHasAccessToDataWithStatusWithoutStatus(List.of("Authorized"),
                List.of("Sensitive"), List.of("Encrypted")));
        ...
        // Requirement #76:
        // EVerest should support secure storage of private keys and certificates used for TLS to backend and EV via TPM
        // in its
        ...
        constraints.add(withStatusWithoutStatusNeverFlowsToComponent(List.of("Sensitive"), List.of("Encrypted"),
                List.of("Logs")));
        constraints.add(withStatusWithoutStatusNeverFlowsTo(List.of("Sensitive"), List.of("Encrypted"),
                List.of("LocalStorage", "TPM")));

        // Requirement #81:
        // EVerest shall not sore any privacy sensitive information in a log file. Privacy sensitive information
        // includes
    ...
    public List<ExpectedViolation> getExpectedViolations() {
        return List.of(
                new ExpectedViolation(1, PCMIdentifier.of("_zTBm4DOgEe-thK3JRY5OhA"),
                        ...
                        Map.of("reservation",
                                List.of(new ExpectedCharacteristic("ActorStatus", "Authorized"),
                                        new ExpectedCharacteristic("Status", "Encrypted"),
                ...
                new ExpectedViolation(1, PCMIdentifier.of("_zTBm4TOgEe-thK3JRY5OhA"),
                        ...
                        Map.of("RETURN",
                                List.of(new ExpectedCharacteristic("ActorStatus", "Authorized"),
                                        ...
                                        new ExpectedCharacteristic("Status", "Sensitive"),
                                        new ExpectedCharacteristic("Status", "Anonymized")),
                                "reservation_id", List.of(new ExpectedCharacteristic("ActorStatus", "Authorized")),
                                "reservation",
                                List.of(new ExpectedCharacteristic("ActorStatus", "Authorized"),
                                        new ExpectedCharacteristic("Status", "Encrypted"),
                                        new ExpectedCharacteristic("Status", "Sensitive")),
                ...
                new ExpectedViolation(5, PCMIdentifier.of("_oCeSgVY4Ee-lU5UkonmXoQ"),
                        List.of(new ExpectedCharacteristic("Location", "ChargingStation"),
                                new ExpectedCharacteristic("Component", "Logs")),
                        Map.of("logmessage",
                                List.of(new ExpectedCharacteristic("ActorStatus", "Authorized"),
                                        new ExpectedCharacteristic("Status", "Token"),
                                        new ExpectedCharacteristic("Status", "Sensitive")))),
                ...
                new ExpectedViolation(5, PCMIdentifier.of("_YLIkwUpjEe-QePQiuhxJfA"),
                        ...
                        Map.of("logmessage",
                                ...
                                List.of(new ExpectedCharacteristic("ActorStatus", "Authorized"),
                                        ...
                                        new ExpectedCharacteristic("Status", "Sensitive")))),
                new ExpectedViolation(5, PCMIdentifier.of("_oCeSgVY4Ee-lU5UkonmXoQ"),
                        List.of(new ExpectedCharacteristic("Component", "Logs"),
                ...
                new ExpectedViolation(6, PCMIdentifier.of("_nDVhIT7IEe-HHLw-F3Rezw"),
                        List.of(new ExpectedCharacteristic("Component", "API"),
                                new ExpectedCharacteristic("Location", "ChargingStation"),
                                new ExpectedCharacteristic("Location", "ChargingStation")),
                ...
                new ExpectedViolation(14, PCMIdentifier.of("_XQriUU0OEe-j2Yo8i_9OIA"),
                        ...
                        Map.of("request", List.of(new ExpectedCharacteristic("ActorStatus", "Authorized")),
                                "firmwareFile",
                                List.of(new ExpectedCharacteristic("Status", "Signed"),
                                        new ExpectedCharacteristic("Status", "Firmware")))),
