...
public class LabelPropagationTest extends BaseTest {
  ...
  public void travelPlannerCharacteristicsPresentTest() {
    ...
    for (CharacteristicsData characteristicData :
        LabelPropagationCharacteristics.travelPlannerCharacteristics) {
      assertTrue(
          propagatedFlowGraph.getPartialFlowGraphs().size()
              >= characteristicData.getSequenceIndex());

      assertCharacteristicPresent(
          propagatedFlowGraph.getPartialFlowGraphs().get(characteristicData.getSequenceIndex()),
          characteristicData.getElementIndex(),
          characteristicData.getVariable(),
          characteristicData.getCharacteristicType(),
  ...
  public void onlineShopCharacteristicsPresentTest() {
    ...
    for (CharacteristicsData characteristicData :
        LabelPropagationCharacteristics.onlineShopCharacteristics) {
      assertTrue(
          propagatedFlowGraph.getPartialFlowGraphs().size()
              >= characteristicData.getSequenceIndex());
  ...
  public void travelPlannerCharacteristicsAbsentTest() {
    ...
    assertCharacteristicAbsent(
        propagatedFlowGraph.getPartialFlowGraphs().get(0), 2, "ccd", "AssignedRoles", "User");
    assertCharacteristicAbsent(
        propagatedFlowGraph.getPartialFlowGraphs().get(0), 6, "RETURN", "GrantedRoles", "User");
    assertCharacteristicAbsent(
        propagatedFlowGraph.getPartialFlowGraphs().get(0), 6, "RETURN", "GrantedRoles", "Airline");
  }
  ...
  public void onlineShopCharacteristicsAbsentTest() {
    ...
    assertCharacteristicAbsent(
        propagatedFlowGraph.getPartialFlowGraphs().get(1),
        0,
        "RETURN",
        "DataSensitivity",
        "Public");
