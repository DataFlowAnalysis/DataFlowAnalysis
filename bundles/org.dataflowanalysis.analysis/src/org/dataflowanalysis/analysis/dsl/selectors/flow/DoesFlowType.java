...
public class DoesFlowType extends AbstractFlowType {
    private static final Logger logger = LoggerManager.getLogger(DoesFlowType.class);

    public DoesFlowType(AbstractSelector sourceSelector, AbstractSelector destinationSelector,
    ...
    public boolean evaluateFlowType(AbstractVertex<?> vertex, DSLConstraintTrace constraintTrace, DSLContext context) {
        ...
        boolean matchedDestination = destinationSelector.matchesDestination(vertex, constraintTrace);
        ...
        if (alternateNodes.isEmpty()) {
            logger.debug(String.format(FAILED_MATCHING_MESSAGE, vertex, destinationSelector));
            return false;
        }
