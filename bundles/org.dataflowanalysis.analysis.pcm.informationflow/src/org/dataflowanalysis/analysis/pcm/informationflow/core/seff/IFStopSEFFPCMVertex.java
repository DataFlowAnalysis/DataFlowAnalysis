 ...
 * {@link StopAction}. If the {@code StopAction} is directly in the SEFF of a {@link GuardedBranchTransition} this
 * vertex also handles the returning evaluation of the logically following {@code GuardedBranchTransition}. Note, this
 * is only relevant in case of handling implicit flow.
 */
public class IFStopSEFFPCMVertex extends AbstractIFSEFFPCMVertex<StopAction> {

    /**
     * As for a {@link SEFFPCMVertex} the vertex has an underlying StopAction SEFF element which influences the behavior
     ...
     */
    public IFStopSEFFPCMVertex(StopAction element, List<? extends AbstractPCMVertex<?>> previousElements, Deque<AssemblyContext> context,
    ...
    @Override
    protected AbstractIFSEFFPCMVertex<StopAction> createIFSEFFVertex(StopAction element, List<? extends AbstractPCMVertex<?>> previousElements,
    ...
    protected List<DataFlowVariable> modifyOutgoingDataFlowVariables(List<DataFlowVariable> outgoingDataFlowVariables) {
        ...
        List<DataFlowVariable> securityContextLayers = IFSecurityContextUtils.getAllSecurityContextLayers(outgoingDataFlowVariables);
