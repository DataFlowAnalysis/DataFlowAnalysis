...
public class DFDVertex extends AbstractVertex<EObject> {
  ...
  public void evaluateDataFlow() {
      ...
      for (var prevVertex : this.getMapPinToPreviousVertex().values()) {
        for (var dfv : prevVertex.getAllOutgoingDataFlowVariables()) {
          if (dfv.getVariableName()
              .equals(this.getMapPinToInputFlow().get(pin).getSourcePin().getId())) {
    ...
    for (var assignment : node.getBehaviour().getAssignment()) {
      List<Label> incomingLabels =
          combineLabelsOnAllInputPins(assignment, mapInputPinsToIncomingLabels);
      ...
      if (assignment instanceof ForwardingAssignment) {
        ...
        mapOutputPinToOutgoingLabels
            .get(assignment.getOutputPin())
  ...
  private static List<Label> combineLabelsOnAllInputPins(
      AbstractAssignment assignment, Map<Pin, List<Label>> mapInputPinsToIncomingLabels) {
  ...
  private static boolean evaluateTerm(Term term, List<Label> inputLabel) {
    if (term instanceof TRUE) {
      ...
      if (binaryTerm instanceof AND) {
        return evaluateTerm(binaryTerm.getTerms().get(0), inputLabel)
            && evaluateTerm(binaryTerm.getTerms().get(1), inputLabel);
  ...
  public boolean isEqual(DFDVertex vertex) {
    ...
    for (var key : this.getMapPinToPreviousVertex().keySet()) {
      if (!this.getMapPinToPreviousVertex()
          .get(key)
          .isEqual(vertex.getMapPinToPreviousVertex().get(key))) return false;
    }
    return true;
  }
  ...
  public DFDVertex clone() {
    ...
    DFDVertex clone =
        new DFDVertex(
            this.name, this.node, newMapPinToPreviousVertex, new HashMap<>(this.mapPinToInputFlow));
    return clone;
  }
