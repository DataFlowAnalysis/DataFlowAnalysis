...
public class PCMFlowGraph extends FlowGraph {
  ...
  public List<AbstractPartialFlowGraph> findPartialFlowGraphs() {
    if (!(super.resourceProvider instanceof PCMResourceProvider)) {
      logger.error(
          "Cannot find partial flow graphs from non-pcm resource provider",
          new IllegalArgumentException(
              "Cannot find partial flow graphs with non-pcm resource provider"));
    }
  ...
  public PCMFlowGraph evaluate() {
    List<PCMPartialFlowGraph> actionSequences =
        this.getPartialFlowGraphs().parallelStream()
            .map(PCMPartialFlowGraph.class::cast)
            .collect(Collectors.toList());
    List<AbstractPartialFlowGraph> evaluatedPartialFlowGraphs =
        actionSequences.parallelStream().map(it -> it.evaluate()).toList();
