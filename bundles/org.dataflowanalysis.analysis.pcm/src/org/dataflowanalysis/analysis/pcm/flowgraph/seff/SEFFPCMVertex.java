...
public class SEFFPCMVertex<T extends AbstractAction> extends AbstractPCMVertex<T> {
  ...
  public void evaluateDataFlow() {
    ...
    if (this.getReferencedElement() instanceof StartAction) {
      List<String> variableNames =
          this.getParameter().stream()
              ...
              .collect(Collectors.toList());
      incomingDataFlowVariables =
          incomingDataFlowVariables.stream()
              .filter(it -> variableNames.contains(it.variableName()))
              .collect(Collectors.toList());
      this.setPropagationResult(
          incomingDataFlowVariables, incomingDataFlowVariables, nodeCharacteristics);
      return;
    } else if (this.getReferencedElement() instanceof StopAction) {
      List<DataFlowVariable> outgoingDataFlowVariables =
          incomingDataFlowVariables.parallelStream()
              .filter(it -> it.getVariableName().equals("RETURN"))
              .collect(Collectors.toList());
      this.setPropagationResult(
          incomingDataFlowVariables, outgoingDataFlowVariables, nodeCharacteristics);
      return;
    } else if (!(this.getReferencedElement() instanceof SetVariableAction)) {
      logger.error(
          "Found unexpected sequence element of unknown PCM type "
              + this.getReferencedElement().getClass().getName());
    ...
    List<ConfidentialityVariableCharacterisation> variableCharacterisations =
        ((SetVariableAction) this.getReferencedElement())
            .getLocalVariableUsages_SetVariableAction().stream()
    ...
    List<DataFlowVariable> outgoingDataFlowVariables =
        super.getDataFlowVariables(
            nodeCharacteristics, variableCharacterisations, incomingDataFlowVariables);
    this.setPropagationResult(
        incomingDataFlowVariables, outgoingDataFlowVariables, nodeCharacteristics);
  }
  ...
  public boolean isBranching() {
    Optional<BranchAction> branchAction =
        PCMQueryUtils.findParentOfType(this.getReferencedElement(), BranchAction.class, false);
  ...
  public String toString() {
    ...
    if (this.getReferencedElement() instanceof StartAction) {
      Optional<ResourceDemandingSEFF> seff =
          PCMQueryUtils.findParentOfType(
              this.getReferencedElement(), ResourceDemandingSEFF.class, false);
      if (seff.isPresent()) {
      ...
      if (this.isBranching()) {
        Optional<BranchAction> branchAction =
            PCMQueryUtils.findParentOfType(this.getReferencedElement(), BranchAction.class, false);
        Optional<AbstractBranchTransition> branchTransition =
            PCMQueryUtils.findParentOfType(
                this.getReferencedElement(), AbstractBranchTransition.class, false);
        elementName =
            "Branching "
                + seff.get().getDescribedService__SEFF().getEntityName()
                + "."
                + branchAction.get().getEntityName()
                + "."
    ...
    return String.format(
        "%s (%s, %s))",
        this.getClass().getSimpleName(), elementName, this.getReferencedElement().getId());
  }

  @Override
  public AbstractPCMVertex<?> deepCopy(
    ...
    SEFFPCMVertex<?> copy =
        new SEFFPCMVertex<>(
            referencedElement,
            List.of(),
            new ArrayDeque<>(context),
            new ArrayList<>(this.getParameter()),
            resourceProvider);
    if (this.isEvaluated()) {
      copy.setPropagationResult(
          this.getAllIncomingDataFlowVariables(),
          this.getAllOutgoingDataFlowVariables(),
          this.getVertexCharacteristics());
    }
    isomorphism.put(this, copy);

    List<AbstractPCMVertex<?>> clonedPreviousElements =
        this.previousElements.stream()
