...
public class DFDDataFlowAnalysisBuilder extends DataFlowAnalysisBuilder {
  ...
  private DFDResourceProvider getEffectiveResourceProvider() {
    return this.customResourceProvider.orElse(
        new DFDURIResourceProvider(
  ...
  protected void validate() {
    ...
    if (this.dataDictionaryPath == null || this.dataDictionaryPath.isEmpty()) {
      logger.error(
          "A data dictionary is required to run the data flow analysis",
    ...
    if (this.dataFlowDiagramPath == null || this.dataFlowDiagramPath.isEmpty()) {
      logger.error(
          "A data flow diagram is required to run the data flow analysis",
  ...
  public DFDConfidentialityAnalysis build() {
    this.validate();
    return new DFDConfidentialityAnalysis(
        this.getEffectiveResourceProvider(), this.pluginActivator, this.modelProjectName);
  }
}
