...
public class ConfidentialityVariableCharacterisationTestUtils {
    ...
    public static boolean evaluateConfidentialityCharacterisationLatticeMapping(List<ConfidentialityVariableCharacterisation> characterisations,
        ...
        for (var characterisation : characterisations) {
            ...
            String setLevelName = lhs.getLiteral()
                    .getName();
            boolean isSet = evaluateConfidentialityCharacterisationForBooleanMapping(characterisation, variableToLiteralToBoolean);
    ...
    public static boolean evaluateConfidentialityCharacterisationForBooleanMapping(ConfidentialityVariableCharacterisation characterisation,
    ...
    private static boolean evaluateTerm(Term term, Map<String, Map<String, Boolean>> nameAndLiteralToBoolean) {
        if (term instanceof True) {
            ...
            return evaluateTerm(andTerm.getLeft(), nameAndLiteralToBoolean) && evaluateTerm(andTerm.getRight(), nameAndLiteralToBoolean);
        } else if (term instanceof Or orTerm) {
            return evaluateTerm(orTerm.getLeft(), nameAndLiteralToBoolean) || evaluateTerm(orTerm.getRight(), nameAndLiteralToBoolean);
    ...
    private static boolean evaluateVariable(NamedEnumCharacteristicReference namedReference,
        ...
        String variableName = namedReference.getNamedReference()
                .getReferenceName();
        ...
        String litteralName = namedReference.getLiteral()
                .getName();
    ...
    private static void throwMissingLiteralMapping(String variableName, String litteralName) {
        String errorMessage = "The litteral '" + litteralName + "' is missing in the mapping for the variable '" + variableName + "'.";
        logger.error(errorMessage);
        throw new IllegalArgumentException(errorMessage);
    }
    ...
    private static String lhsAsString(AbstractNamedReference var, LhsEnumCharacteristicReference lhs) {
        return var.getReferenceName() + "." + lhs.getCharacteristicType()
                .getName() + "."
                + lhs.getLiteral()
                        .getName();
    }
    ...
    private static String termAsString(Term term) {
        if (term instanceof True) {
            ...
            return namedReference.getNamedReference()
                    .getReferenceName() + "."
                    + namedReference.getCharacteristicType()
                            .getName()
