...
public class IFConfidentialityVariableCharacterisationUtils {
	...
	public static List<ConfidentialityVariableCharacterisation> resolveWildcards(
		...
		return resolveCharacteristicTypeWildcard(characterisation, allCharacteristicTypes).stream().flatMap(
				confidentialityCharacterisation -> resolveLiteralWildcard(confidentialityCharacterisation).stream())
				.toList();
	}
	...
	public static List<ConfidentialityVariableCharacterisation> resolveLiteralWildcard(
		...
		for (Literal level : enumCharacteristicType.getType().getLiterals()) {
			...
			List<NamedEnumCharacteristicReference> variablesInRhs = extractAllVariables(
					resolvedCharacterisation.getRhs());
			variablesInRhs.stream().forEach(variable -> variable.setLiteral(level));
	...
	public static List<ConfidentialityVariableCharacterisation> createModifiedCharacterisationsForAdditionalHigherEqualConstraint(
		...
		for (Literal latticeLevel : lattice.getLiterals()) {
			modifiedCharacterisations.add(createModifiedCharacterisationForLevel(characterizedVariable,
					literalToDefinedConfidentialityCharacterisation, constraint, latticeLevel,
	...
	private static ConfidentialityVariableCharacterisation createModifiedCharacterisationForLevel(
		...
		return createCharacterisation(constraint, latticeCharacteristicType, level,
				createRhsForAdditionalHigherEqualConstraint(lowerDefinedTerms, levelTerm, levelConstraintVariable,
						higherConstraintVariables));
	}
	 ...
	 * the lowest level of the lattice.
	 * 
	...
	public static List<ConfidentialityVariableCharacterisation> createSetLowestLevelCharacterisationsForLattice(
			AbstractNamedReference characterisedVariable, CharacteristicType latticeCharacteristicType,
			Enumeration lattice) {
		...
		for (Literal level : lattice.getLiterals()) {
			if (level.equals(lowestLevel)) {
				characterisations.add(createTrueCharacteristationForLevel(characterisedVariable,
						latticeCharacteristicType, lowestLevel));
			} else {
				characterisations.add(createFalseCharacteristationForLevel(characterisedVariable,
						latticeCharacteristicType, lowestLevel));
			}
		}
	...
	private static ConfidentialityVariableCharacterisation createTrueCharacteristationForLevel(
			AbstractNamedReference characterisedVariable, CharacteristicType latticeCharacteristicType, Literal level) {
	...
	private static ConfidentialityVariableCharacterisation createFalseCharacteristationForLevel(
			AbstractNamedReference characterisedVariable, CharacteristicType latticeCharacteristicType, Literal level) {
	...
	private static ConfidentialityVariableCharacterisation createMaximumJoinCharacterisationForLevel(
		...
		return createCharacterisation(characterisedVariable, latticeCharacteristicType, level,
				createRhs(levelDependencies, higherDependencies));
	}
	...
	private static Term copyTerm(Term term) {
		if (term instanceof True) {
			...
			return createNamedEnumCharacteristicReference(namedReference.getNamedReference(),
					namedReference.getCharacteristicType(), namedReference.getLiteral());
