...
public class PCMNodeCharacteristicsCalculator implements VertexCharacteristicsCalculator {
  ...
  public List<CharacteristicValue> getNodeCharacteristics(
      Entity node, Deque<AssemblyContext> context) {
    ...
    List<EnumCharacteristic> enumCharacteristics =
        assignees.stream()
    ...
    return enumCharacteristics.stream()
        .flatMap(
            it -> it.getValues().stream().map(val -> new PCMCharacteristicValue(it.getType(), val)))
        .collect(Collectors.toList());
  }
  ...
  private List<AbstractAssignee> getUsage(Entity node, Assignments assignments) {
    UsageScenario usageScenario =
        PCMQueryUtils.findParentOfType(node, UsageScenario.class, false).get();
  ...
  private List<AbstractAssignee> getSEFF(Assignments assignments, Deque<AssemblyContext> context) {
    ...
    var allocations =
        this.resourceLoader
            .lookupToplevelElement(AllocationPackage.eINSTANCE.getAllocation())
            .stream()
    ...
    Optional<Allocation> allocation =
        allocations.stream()
            .filter(
                it ->
                    it.getAllocationContexts_Allocation().stream()
        ...
        .forEach(
            it -> {
              resolvedAssignees.addAll(
                  getAllocation(assignments, it.getAssemblyContext_AllocationContext()));
              resolvedAssignees.addAll(
                  getResource(assignments, it.getResourceContainer_AllocationContext()));
              if (it.getAssemblyContext_AllocationContext()
                ...
                resolvedAssignees.addAll(
                    getComposite(
                        ...
                        context,
                        (CompositeComponent)
                            it.getAssemblyContext_AllocationContext()
                                .getEncapsulatedComponent__AssemblyContext()));
              }
            });
   ...
   * @return List of resolved assignees matching the node
   */
  private List<AbstractAssignee> getAllocation(
      Assignments assignments, AssemblyContext assemblyContext) {
    return assignments.getAssignee().stream()
   ...
   * @return List of resolved assignees matching the node
   */
  private List<AbstractAssignee> getResource(
      Assignments assignments, ResourceContainer resourceContainer) {
    return assignments.getAssignee().stream()
   ...
   */
  private List<AbstractAssignee> getComposite(
  ...
  private Assignments resolveAssignments() {
    return this.resourceLoader
        .lookupToplevelElement(NodeCharacteristicsPackage.eINSTANCE.getAssignments())
        .stream()
  ...
  private boolean presentInAssembly(AssemblyContext assemblyContext) {
    List<System> systems =
        this.resourceLoader
            .lookupToplevelElement(SystemPackage.eINSTANCE.getSystem())
            .parallelStream()
  ...
  private boolean presentInComposite(AssemblyContext assemblyContext) {
    List<Repository> repositories =
        this.resourceLoader
            ...
            .collect(Collectors.toList());
    List<CompositeComponent> compositeCompontents =
  ...
  public void checkCharacteristics(List<EnumCharacteristic> characteristics) {
    for (EnumCharacteristic characteristic : characteristics) {
      ...
      List<Literal> unknownLiterals =
          foundLiterals.parallelStream()
              .filter(it -> !allowedLiterals.contains(it))
              .collect(Collectors.toList());
      if (!unknownLiterals.isEmpty()) {
        throw new IllegalStateException(
            "Found unknown literal "
                + unknownLiterals.get(0).getName()
                + " in assigned characteristics!");
      }
    }
  }
}
