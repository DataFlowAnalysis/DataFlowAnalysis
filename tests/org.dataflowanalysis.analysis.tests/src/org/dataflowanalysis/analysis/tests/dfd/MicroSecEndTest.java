...
public class MicroSecEndTest {
    ...
    public DFDConfidentialityAnalysis buildAnalysis(String name) {
        ...
        return new DFDDataFlowAnalysisBuilder().standalone().modelProjectName(PROJECT_NAME).usePluginActivator(Activator.class)
    ...
    public void convertAllToWeb() throws StandaloneInitializationException {
        ...
        for (String model : models) {
            ...
            var web = converter.dfdToWeb(PROJECT_NAME, Paths.get(location, model + ".dataflowdiagram").toString(),
                    Paths.get(location, model + ".datadictionary").toString(), Activator.class);
    ...
    private boolean hasDataCharacteristic(AbstractVertex<?> node, String type, String value) {
        return node.getAllDataFlowVariables().stream()
                .anyMatch(v -> v.getAllCharacteristics().stream().anyMatch(c -> c.getTypeName().equals(type) && c.getValueName().equals(value)));
    }
