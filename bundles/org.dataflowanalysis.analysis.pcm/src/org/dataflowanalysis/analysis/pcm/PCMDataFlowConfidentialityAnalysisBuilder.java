...
public class PCMDataFlowConfidentialityAnalysisBuilder extends DataFlowAnalysisBuilder {
   ...
   */
  public PCMDataFlowConfidentialityAnalysisBuilder usePluginActivator(
   ...
   * @return Returns builder object of the analysis
   */
  public PCMDataFlowConfidentialityAnalysisBuilder useAllocationModel(
   ...
   * @return Returns builder object of the analysis
   */
  public PCMDataFlowConfidentialityAnalysisBuilder useCustomResourceProvider(
  ...
  private PCMResourceProvider getURIResourceProvider() {
    return new PCMURIResourceProvider(
  ...
  protected void validate() {
    ...
    if (this.customResourceProvider.isEmpty()
      ...
      logger.error(
          "The dataflow analysis requires a path to a usage model",
          new IllegalStateException("The Analysis requires a usage model"));
    }
    if (this.customResourceProvider.isEmpty()
        && (this.relativeAllocationModelPath == null
            || this.relativeAllocationModelPath.isEmpty())) {
      logger.error(
          "The dataflow analysis requires a path to an allocation model",
    ...
    if (this.customResourceProvider.isPresent()) {
      ...
      if (!this.customResourceProvider.get().sufficientResourcesLoaded()) {
        logger.error(
            "The custom resource provider could not load all required resources",
            new IllegalStateException("Could not load all required resources"));
      }
    }
    if (this.relativeNodeCharacteristicsPath == null
        || this.relativeNodeCharacteristicsPath.isEmpty()) {
      logger.warn(
          "Using node characteristic model without specifying path to the assignment model. No node"
  ...
  public PCMDataFlowConfidentialityAnalysis build() {
    ...
    return new PCMDataFlowConfidentialityAnalysis(
        resourceProvider, this.modelProjectName, this.pluginActivator);
  }
}
