...
public class PCMSEFFFinderUtils {
  ...
  public static List<PCMPartialFlowGraph> findSequencesForSEFFAction(
    ...
    if (currentAction instanceof StartAction) {
      return findSequencesForSEFFStartAction(
          (StartAction) currentAction, context, previousSequence, resourceProvider);

    } else if (currentAction instanceof StopAction) {
      return findSequencesForSEFFStopAction(
          (StopAction) currentAction, context, previousSequence, resourceProvider);

    } else if (currentAction instanceof ExternalCallAction) {
      return findSequencesForSEFFExternalCallAction(
          (ExternalCallAction) currentAction, context, previousSequence, resourceProvider);

    } else if (currentAction instanceof SetVariableAction) {
      return findSequencesForSEFFSetVariableAction(
          (SetVariableAction) currentAction, context, previousSequence, resourceProvider);

    } else if (currentAction instanceof BranchAction) {
      return findSequencesForSEFFBranchAction(
          (BranchAction) currentAction, context, previousSequence, resourceProvider);

    } else {
      // default case: skip action and continue with successor
      logger.info(
          String.format(
              "Action %s has unsupported type of %s and is skipped.",
              currentAction.getId(), currentAction.getClass().getName()));
      return findSequencesForSEFFAction(
          currentAction.getSuccessor_AbstractAction(), context, previousSequence, resourceProvider);
    }
  }

  private static List<PCMPartialFlowGraph> findSequencesForSEFFStartAction(
      StartAction currentAction,
      SEFFFinderContext context,
      PCMPartialFlowGraph previousSequence,
      ResourceProvider resourceProvider) {
    var startElement =
        new SEFFPCMVertex<StartAction>(
            currentAction,
            List.of(previousSequence.getSink()),
            context.getContext(),
            context.getParameter(),
            resourceProvider);
    var currentSequence = new PCMPartialFlowGraph(startElement);
    return findSequencesForSEFFAction(
        currentAction.getSuccessor_AbstractAction(), context, currentSequence, resourceProvider);
  }

  private static List<PCMPartialFlowGraph> findSequencesForSEFFStopAction(
      StopAction currentAction,
      SEFFFinderContext context,
      PCMPartialFlowGraph previousSequence,
      ResourceProvider resourceProvider) {
    var stopElement =
        new SEFFPCMVertex<StopAction>(
            currentAction,
            List.of(previousSequence.getSink()),
            context.getContext(),
            context.getParameter(),
    ...
    Optional<AbstractAction> parentAction =
        PCMQueryUtils.findParentOfType(currentAction, AbstractAction.class, false);
  ...
  private static List<PCMPartialFlowGraph> findSequencesForSEFFExternalCallAction(
    ...
    Optional<SEFFWithContext> calledSEFF =
        PCMQueryUtils.findCalledSEFF(calledRole, calledSignature, context.getContext());

    if (calledSEFF.isEmpty()) {
      return List.of(previousSequence);
    } else {
      Optional<StartAction> SEFFStartAction =
          PCMQueryUtils.getFirstStartActionInActionList(
              calledSEFF.get().seff().getSteps_Behaviour());
      ...
      if (SEFFStartAction.isEmpty()) {
        ...
        return findSequencesForSEFFAction(
            SEFFStartAction.get(), context, currentActionSequence, resourceProvider);
      }
    }
  }

  private static List<PCMPartialFlowGraph> findSequencesForSEFFSetVariableAction(
    ...
    return findSequencesForSEFFAction(
        currentAction.getSuccessor_AbstractAction(),
        context,
        currentActionSequence,
        resourceProvider);
  }

  private static List<PCMPartialFlowGraph> findSequencesForSEFFBranchAction(
      BranchAction currentAction,
      SEFFFinderContext context,
      PCMPartialFlowGraph previousSequence,
      ResourceProvider resourceProvider) {
    var flowGraph =
        currentAction.getBranches_Branch().stream()
            ...
            .map(
                it -> {
                  Map<AbstractPCMVertex<?>, AbstractPCMVertex<?>> isomorphism =
                      new IdentityHashMap<>();
                  ...
                  return findSequencesForSEFFAction(
                      it, clonedContext, clonedSequence, resourceProvider);
                })
            .flatMap(List::stream)
            .toList();
    return flowGraph;
  ...
  private static List<PCMPartialFlowGraph> findSequencesForSEFFActionReturning(
      ExternalCallAction currentAction,
      SEFFFinderContext context,
      PCMPartialFlowGraph previousSequence,
      ResourceProvider resourceProvider,
  ...
  public static List<PCMPartialFlowGraph> returnToCaller(
      AbstractVertex<?> caller,
      SEFFFinderContext context,
      PCMPartialFlowGraph previousSequence,
      ResourceProvider resourceProvider) {
    ...
    if (caller instanceof CallingUserPCMVertex) {
      return returnToUserCaller(
          (CallingUserPCMVertex) caller, context, previousSequence, resourceProvider);

    } else if (caller instanceof CallingSEFFPCMVertex) {
      return returnToSEFFCaller(
          (CallingSEFFPCMVertex) caller, context, previousSequence, resourceProvider);

    } else {
      throw new IllegalArgumentException(
          String.format(
              "No dispatch logic for call of type %s available.",
              caller.getClass().getSimpleName()));
    }
  }

  private static List<PCMPartialFlowGraph> returnToUserCaller(
      CallingUserPCMVertex caller,
      SEFFFinderContext context,
      PCMPartialFlowGraph previousSequence,
      ResourceProvider resourceProvider) {
    if (!context.getCallers().isEmpty()) {
      ...
      return PCMUserFinderUtils.findSequencesForUserActionReturning(
          caller.getReferencedElement(), previousSequence, resourceProvider, caller);
    }
  }

  private static List<PCMPartialFlowGraph> returnToSEFFCaller(
      CallingSEFFPCMVertex caller,
      SEFFFinderContext context,
      PCMPartialFlowGraph previousSequence,
      ResourceProvider resourceProvider) {
    context.updateSEFFContext(caller.getContext());
    return findSequencesForSEFFActionReturning(
        caller.getReferencedElement(), context, previousSequence, resourceProvider, caller);
  }
}
