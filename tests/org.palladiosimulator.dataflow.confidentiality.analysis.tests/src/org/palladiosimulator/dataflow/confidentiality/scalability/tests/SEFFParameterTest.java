...
public class SEFFParameterTest implements ScalibilityTest {
	...
	public void run(ScalibilityParameter parameter) {
		...
		try {
			factory = new PCMModelFactory("../org.palladiosimulator.dataflow.confidentiality.analysis.testmodels/SEFFParameterTest", true,
					Activator.class, AnalysisUtils.TEST_MODEL_PROJECT_NAME);
		} catch (IOException e) {
			logger.error("Unable to create model factory", e);
			return;
		}
		ResourceContainer resourceContainer = factory.addResourceContainer("SEFFServer");
		DataType dataType = factory.addDataType("SEFFDataType");
		InterfaceBuilder interfaceBuilder = InterfaceBuilder.builder(factory.getRepository())
				.setName("SEFFInterface");
