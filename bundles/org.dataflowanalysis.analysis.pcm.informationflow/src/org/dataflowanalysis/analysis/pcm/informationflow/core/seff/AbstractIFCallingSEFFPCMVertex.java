...
/**
 * A vertex for the evaluation of information flows. In comparison to {@link CallingSEFFPCMVertex}
...
public abstract class AbstractIFCallingSEFFPCMVertex extends CallingSEFFPCMVertex implements IFConfigurablePCMVertex {
    ...
    /*
     * Note, most of the defined behavior in this class is nearly duplicated in AbstractIFSEFFPCMVertex and
    ...
    /**
     * As for a {@link CallingSEFFPCMVertex} the vertex has an underlying {@code ExternalCallAction} SEFF element which
     * influences the behavior through defined VariableCharacterisations. The vertex can have {@code previousElements} from
     * which the incoming DataFlowVariables are received. Furthermore, the vertex contains an {@link AssemblyContext},
     * passed {@link Parameter}s as well as a {@link ResourceProvider}. Lastly, the vertex might consider implicit flow and
     ...
     * @param isCalling whether the vertex stands for the calling or returning behavior
     ...
     */
    public AbstractIFCallingSEFFPCMVertex(ExternalCallAction element, List<? extends AbstractPCMVertex<?>> previousElements,
            Deque<AssemblyContext> context, List<Parameter> parameter, boolean isCalling, ResourceProvider resourceProvider,
    ...
    public AbstractPCMVertex<?> deepCopy(Map<AbstractPCMVertex<?>, AbstractPCMVertex<?>> isomorphism) {
        ...
        AbstractIFCallingSEFFPCMVertex copy = createIFSEFFVertex(getReferencedElement(), List.copyOf(previousElements),
                new ArrayDeque<>(getContext()), List.copyOf(getParameter()), resourceProvider, considerImplicitFlow, extractionStrategy);
     ...
     * @see #AbstractIFCallingSEFFPCMVertex(ExternalCallAction, List, Deque, List, boolean, ResourceProvider, boolean,
     * IFPCMExtractionStrategy)
     */
    protected abstract AbstractIFCallingSEFFPCMVertex createIFSEFFVertex(ExternalCallAction element,
            List<? extends AbstractPCMVertex<?>> previousElements, Deque<AssemblyContext> context, List<Parameter> parameter,
            ResourceProvider resourceProvider, boolean considerImplicitFlow, IFPCMExtractionStrategy extractionStrategy);
