...
public class UsageCallBuilder {
	...
	public UsageCallBuilder addInputCharacteristic(EnumCharacteristicType characteristicType, String characteristicValue) {
	...
	public UsageCallBuilder addOutputCharacteristic(EnumCharacteristicType characteristicType, String characteristicValue) {
		Literal literal = characteristicType.getType().getLiterals().stream()
				.filter(it -> it.getName().equalsIgnoreCase(characteristicValue))
				.findAny().orElseThrow(() -> new IllegalArgumentException("Unknown characteristic value"));
		VariableUsage usage = ParameterFactory.eINSTANCE.createVariableUsage();
